...
public class WXPayUtil {
    ...
    public static String pushClient(HttpServletRequest request, HttpServletResponse response, User user, int money,String orderNo) throws Exception {
        ...
        queryReq.setParameter("openid", user.getUserWechatopenid());
        queryReq.setParameter("partner_trade_no",orderNo);
        queryReq.setParameter("re_user_name", user.getUserName());//可选
        System.out.println("没有String实例化"+user.getUserName()+"+实例化"+new String(user.getUserName()));
