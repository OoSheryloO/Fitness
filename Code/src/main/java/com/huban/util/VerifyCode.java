...
public class VerifyCode {
	...
	public boolean sendCheckNumber(String mobile, String checkNumber, ErrorsLogService errorLogService) {
		...
		try {
			...
			// TODO: handle exception
			 Errorlogs errlog=new Errorlogs();
	          errlog.setErrorlogNote(e.getMessage()+":"+errormessage);
...
//		SendSmsResponse sendSmsResponse = acsClient.getAcsResponse(request);
//		if (sendSmsResponse.getCode() != null && sendSmsResponse.getCode().equals("OK")) {System.out.println("进入判断");
//			return true;
//		}
...
//    		System.out.println("发送失败！！！！！！！！！！！！！");
			// TODO: handle exception
    		//{"error_response":{"code":15,"msg":"Remote service error","sub_code":"isv.AMOUNT_NOT_ENOUGH","sub_msg":"余额不足","request_id":"3gsm4zp7qkjm"}}
//          e.printStackTrace();
//          Errorlogs errlog=new Errorlogs();
...
//       TaobaoClient client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", ConstantUtil.VerifyCodeAppID, ConstantUtil.VerifyCodeSecretKey);// 接口, AppKey 应用的唯一标识, 应用的签名密钥
//    	//TaobaoClient client = new DefaultTaobaoClient("https://help.aliyun.com/document_detail/55284.html?spm=5176.sms-account.109.1.66e3621I1ZESp", ConstantUtil.VerifyCodeAppID, ConstantUtil.VerifyCodeSecretKey);// 接口, AppKey 应用的唯一标识, 应用的签名密钥
...
//        request.setSmsFreeSignName(ConstantUtil.VerifyCodeSignName);//我帮你托 短信签名:传入的签名必须在在阿里大鱼"管理中心 -> 短信签名管理" 中可用的签名作为短信签名例如:【阿里大鱼】欢迎使用阿里大鱼服务。
//        request.setSmsParam("{\"code\":\"" + checkNumber + "\",\"product\":\""+ ConstantUtil.VerifyCodeProdName +"\"}");//短信模板变量，传参规则{"key":"value"}，key的名字须和申请模板中的变量名一致，多个变量之间以逗号隔开。示例：针对模板“验证码${code}，您正在进行${product}身份验证，打死不要告诉别人哦！”，传参时需传入{"code":"1234","product":"alidayu"}
...
//        request.setSmsTemplateCode(ConstantUtil.VerifyCodeTemplate);//短信模板编号，传入的模板必须是在阿里大鱼"管理中心 -> 短信模板管理"中的可用模板。示例：SMS_585014
//
